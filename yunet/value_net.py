...
class ValueNetBig(nn.Module):
    """价值网络：输入21X21X9Xc的状态，输出1个当前状态的价值"""

    def __init__(self, state_channel, OI):
        super().__init__()

        self.inc = BigConv(state_channel, 8)  # 21x21x9x8
        self.down1 = BigDown(8, 16)  # 10x10x4x16
        self.down2 = BigDown(16, 32)  # 5x5x2x32=1600
        self.fc1 = nn.Linear(1600, 256)
        self.fc2 = nn.Linear(256, 64)
        self.fc3 = nn.Linear(64, 1)
        if OI:
...
class ValueNetStepBig(nn.Module):
    """价值网络：输入21X21X9Xc的状态，输出1个当前状态的价值"""
