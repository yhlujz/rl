...
from monai.data import CacheDataset, DataLoader, decollate_batch, DistributedSampler
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description='3D分割训练')
    parser.add_argument('--epochs', '-e', type=int, default=500, help='训练的epoch数（默认为500）') 
    parser.add_argument('--batch-size', '-b', type=int, default=1, help='训练的Batch size大小（默认为1）') 
    parser.add_argument('--learning-rate', '-l', type=float, default=1e-5, help='训练的学习率（默认为1e-5）', dest='lr')
    parser.add_argument('--num-workers', '-w', type=int, default=0, help='加载进程数（默认为0）')
...
def train(args):
    ...
    # 固定随机种子
    set_determinism(seed=(1 + local_rank))

    # 设置只有0号GPU输出信息
    if local_rank != 0:
        f = open(os.devnull, "w")
        sys.stdout = sys.stderr = f  # 重定向，将print输出重定向到f文件
    ...
    train_loader = DataLoader(
        ...
        shuffle=False,  # 自定义sampler，shuffle必须为false
    ...
    for epoch in range(max_epochs):
        ...
        epoch_loss = 0
        step = 0
        train_sampler.set_epoch(epoch)  # 设置sampler的epoch，使不同进程获得同样shuffle
